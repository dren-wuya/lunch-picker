...
const INTENT_PAYLOAD_KEYS = Object.freeze({
  ...
  saveRecord: ["originalDate", "date", "status", "restaurantId", "source", "reason", "confirmed"],
  deleteRecord: ["date"],
...
export function createView(dispatch) {
  ...
  function populateRestaurantSelect() {
    ...
    const restaurants = [...viewModel.restaurants]
      .sort((left, right) => {
    ...
    restaurants.forEach((restaurant) => {
      ...
      option.textContent = "[" + SCOPE_LABELS[restaurant.scope] + "] "
  ...
  function openRecordDialog(record = null, preferredStatus = "restaurant") {
    ...
    if (record && record.status === "restaurant") {
      const recordRestaurantId = record.restaurantId || "";
      const optionExists = [...elements["record-restaurant"].options]
        .some((option) => option.value === recordRestaurantId);
      if (!optionExists) {
        const orphanOption = document.createElement("option");
        orphanOption.value = recordRestaurantId;
        orphanOption.textContent = (record.restaurantId ? "[已移出目录] " : "[未关联] ")
          + record.restaurantName;
        elements["record-restaurant"].append(orphanOption);
      }
      elements["record-restaurant"].value = recordRestaurantId;
  ...
  function saveRecordFromForm(event) {
    ...
    try {
      ...
      const payload = {
        ...
        status,
        restaurantId: elements["record-restaurant"].value,
